...
plt.figure(figsize=[8, 4])
for i, b in enumerate(bli):
    plt.plot(1e6 * b.t, b.Pg / 1e5, cli[i] + '-', lw=2, label=f"A = {amps[i]:.2f} * p")

plt.xlabel(r'time in $\mu$s')
plt.ylabel("gas pressure inside bubble (bar)")
plt.yscale("log")         # Log scale for pressure
plt.ylim(1e-1, 1e4)       # Optional: Adjust to your pressure range

# Force nice tick formatting on x-axis
plt.ticklabel_format(axis='x', style='plain')  # Prevent scientific notation
plt.xticks(rotation=0)                         # Optional: rotate ticks

plt.legend()
plt.tight_layout()
plt.savefig('Pg_inside_bubble_log' + suffix + '.png', dpi=240)
plt.close()
